...
## Code Validation
  ...
  - ### CSS :heavy_check_mark:
    ...
    > - The only errors found are ```Invalid RGB function``` caused by the use of css variables inside rgb functions. These functions are rendering corectly on every test, therefore, the errors are acceptable and can be ignored. The only warnings are about the vendor prefixes, which are acceptable can be ignored.
  - ### JavaScript :heavy_check_mark:
    > Javascript has been validated during building proccess by **VSCODE** extension **[ESLint](https://eslint.org/)**. **No errors or issues** :heavy_check_mark:.
